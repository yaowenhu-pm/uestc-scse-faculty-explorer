...
界面采用三列教师目录，支持搜索、院系/职称/等级组合筛选和教师证据详情。S/A/B/C/D/E 均在原有门槛内按固定分数线细分为加档、中档、减档，全部 18 档均可筛选；默认先按 S → E，再按各级的加档 → 中档 → 减档排列，同档按证据分降序。设计依据与完整实施步骤见 [界面实现方案](./docs/implementation-plan.md)。
